<?php	
	...
	if (empty($_POST['fullname'])) {
		...
		if ($count>0){
			...
			$sql = "insert into user (fullname,email,password,country,image,created_at) ";
			$sql .= "value (\"$fullname\",\"$email\",\"$password\",\"$country\",\"$default_profile\",$created_at)";

			$query_new_insert = mysqli_query($con,$sql);
					if (empty($_POST['email'])) {
           echo  "<script>alert(\"Correo electrónico invalido\"); window.location=\"../register.php\"</script>";
        } else if (empty($_POST['password'])){
			echo  "<script>alert(\"Contraseña invalida\"); window.location=\"../register.php\"</script>";
		} else if (
			!empty($_POST['email'])  &&
			!empty($_POST['password'])
		){
